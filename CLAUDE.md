...
- **`/clear` vs. a new conversation — not interchangeable.** Both wipe my live context, so the per-turn token cost after either is identical; token saving is *not* what decides between them. The difference is the session record: `/clear` keeps appending to the same transcript file, a new conversation starts a fresh one. That matters twice. (1) `context_check.py` measures the transcript *file* (turns + MB), so `/clear` does not reset its counters — keep clearing in one session and it will read 🔴 while my context is actually empty, a false alarm. (2) One transcript = one version keeps `--resume`/`--continue`/`/rewind` usable. So: a sub-version that ships (has a QA file, a commit, a `HANDOFF.md` update) closes with a **new conversation**, same as a full version — 🟡/🔴 from `context_check.py` always means new conversation, never `/clear`. `/clear` is only for throwaway exploration inside a session that has no record worth keeping.
...
python C:/Users/Shalom/.claude/tools/context_check.py                               # end of every step — 🟢 continue / 🟡🔴 close the conversation
```
...
Use `/clear` only for throwaway exploration with no session record worth keeping (see "Context discipline" — anything that ships closes with a new conversation, not `/clear`), `/context` to inspect context use, `/usage` to monitor the Claude plan, and `/rewind` when an implementation direction is wrong.
